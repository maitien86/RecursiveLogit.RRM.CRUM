%% GRRM LL evaluation
%%
function [LL, grad] = getLL_DeC_CERUM()
    ...
    for i = 1:nbnonzero
        ...
        for j = 1:size(col,2)
            for t = 1: np
                dt = Op.x(2*np + t);
                lambda = dt;
                u =  Op.x(t) * (Atts(t).Value(k,a)- Atts(t).Value(k,col(j))) + Op.x(t+np) * Atts(t).Value(k,col(j));
    ...
    Ufull = sparse(S);
    Mfull = Ufull;
    Mfull(find(incidenceFull)) =  exp(Ufull(find(incidenceFull)));
