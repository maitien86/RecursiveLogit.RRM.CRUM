%% GRRM LL evaluation
%%
function [LL, grad] = getLL_DeC_EGRRM_test()
    ...
    for i = 1:nbnonzero
        ...
        for j = 1:size(col,2)
            for t = 1: np
                ...
                temp = temp + u;% log(lambda + exp(u));
                delta(t) = delta(t) + Atts(t).Value(k,a); %(exp(u) * (Atts(t).Value(k,a)))/(lambda + exp(u));
                delta(t+np) =  delta(t+np) + (cos(dt)/2)/(lambda + exp(u));
            end
%             t = np;
%             dt = Op.x(2*np + t);
%             lambda = (sin(dt)+1)/2;
%             u =   Op.x(t+np) * Atts(t).Value(k,col(j)) + Op.x(t) * (Atts(t).Value(k,col(j)) - Atts(t).Value(k,a));
...
%             delta(t+ 2*np) =  delta(t+ 2*np) + (cos(dt)/2)/(lambda + exp(u)); 
        end
        % Including LC
        S(k,a) = temp/nA;
        for t = 1: dnp
