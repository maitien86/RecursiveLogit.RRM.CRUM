%% GRRM LL evaluation
%%
function [LL, grad] = getLL_DeC_GRRM()
    ...
    for i = 1:nbnonzero
        ...
        for j = 1:size(col,2)
            for t = 1: np
                lambda = Op.x(np + t);
                u =  Op.x(t) * (Atts(t).Value(k,col(j)) - Atts(t).Value(k,a));
                temp = temp + log(lambda + exp(u));
                delta(t) = delta(t) + (exp(u) * (Atts(t).Value(k,col(j)) - Atts(t).Value(k,a)))/(lambda + exp(u));
                %delta(t) = delta(t) - (exp(u) * Atts(t).Value(k,a))/(lambda + exp(u));
                delta(t+np) =  delta(t+np) + 1/(lambda + exp(u));
            end            
        end
        for t = 1:np
            temp = temp - log(Op.x(np+t)+1);
            delta(t+np) = delta(t+np) - 1/(Op.x(t+np)+1);
        end
        S(k,a) = temp;% - log(Op.x(Op.n)+1) * np;
        %delta(dnp) = delta(dnp) - np * 1/(Op.x(Op.n)+1);
        for t = 1: dnp
